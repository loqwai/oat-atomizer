...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 lastBackground = getLastFrameColor(fragCoord).rgb;
  ...
  vec2 distortedUV = uv;
  distortedUV.x += sin(iTime * 0.5f) * 10.f + spectralSpreadZScore + energyZScore;
  distortedUV.y += sin(iTime * 0.2f) * 10.f + spectralSpreadZScore + energyZScore;

  vec3 distortedColor = getLastFrameColor(distortedUV).rgb;
  distortedColor = rotateHue(distortedColor, sin(iTime) * 0.1f);
  if(beat) {
    distortedColor = rotateHue(distortedColor, sin(iTime) * 0.5f);
  }
  ...
  distortedUV.x += cos(iTime * 0.5f);
  distortedUV.y += cos(iTime * 0.2f);
