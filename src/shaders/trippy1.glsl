...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec3 finalColor = vec3(0.0);
		...
		// smoothedEnergy = smoothstep(smoothedEnergy, energy, 0.0001);
		// smoothedSpectralCentroid = smoothstep(smoothedSpectralCentroid, spectralCentroid, 0.1);
    ...
    for (float i = 0.0; i < 4.0; i++) {
        ...
        vec3 col = palette(length(uv0) + i * .4 + iTime * .4 + smoothedSpectralCentroid * 0.005); // Shift color palette with spectral centroid

        d = sin(d * (8. + smoothedSpectralSkewness * 0.5) + iTime) / 8.; // Modify pattern with spectral skewness
        d = abs(d);

        d = pow(0.01 / d, 1.2 + smoothedSpectralKurtosis * 0.01); // Use spectral kurtosis to control the sharpness
